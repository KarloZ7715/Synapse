...
1. **WebGPU > WASM para ML:** WebGPU ofrece 15-30x mejor rendimiento que WebGL y alcanza ~80% del rendimiento nativo. Para inferencia de **modelos ONNX pequeños** (p. ej. TextCNN sobre embeddings), WebGPU es el backend óptimo.
2. **Web Worker = UI no bloqueada:** La carga del modelo ONNX y la inferencia ocurren en un hilo separado. El hilo principal solo recibe los metadatos de clasificación vía `postMessage`.
...
- **Arquitectura:** TextCNN propia + embeddings FastText (300d); 4 cabezas categóricas (multi-task single-label)
- **Entrenamiento:** PyTorch en Colab; pérdida = suma de `CrossEntropyLoss` por cabeza
- **Exportación:** PyTorch → ONNX con `torch.onnx.export` (+ cuantización INT8 opcional)
- **Tamaño estimado:** orden de **< 15 MB** FP32 según tamaño de vocabulario embebido (típicamente **mucho menor** que un DistilBERT)
...
| WASM sin WebGPU             | Significativamente más lento que WebGPU para ML en el cliente |

## Consecuencias

- **Positivas:** Inferencia <100ms, privacidad, sin costos de servidor para clasificación
- **Negativas:** Compatibilidad WebGPU variable por navegador; hay que servir artefactos ONNX + vocabulario alineado con el preprocesado.
- **Mitigación:** Indicador de carga durante descarga del modelo. Fallback a WASM. Ver ADR-008 para el clasificador TextCNN.
